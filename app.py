...
# ── Theme detection & user override ─────────────────────────────────────────
if "theme_override" not in st.session_state:
    st.session_state.theme_override = "auto"  # auto / light / dark

# JavaScript to detect system preference
theme_js = """
<script>
...
with st.sidebar:
    st.markdown("### 🐝 Hive Controls & Theme")
    
    theme_choice = st.radio(
        "Appearance",
        options=["Auto (system)", "Light ☀️", "Dark 🌙"],
        index=0 if st.session_state.theme_override == "auto" else
               1 if st.session_state.theme_override == "light" else 2,
        horizontal=True
    )

    if theme_choice == "Auto (system)":
        st.session_state.theme_override = "auto"
    elif theme_choice == "Light ☀️":
...
<style>
    :root {
        ...
        --text-main: #e0e0ff;
        --accent: #ffcc33;
        --accent-glow: #ffaa0066;
        --chat-user-bg: linear-gradient(135deg, #2a1f00, #3a2f00);
        --chat-assist-bg: linear-gradient(135deg, #1a1a2e, #0f0f1e);
        --input-bg: #111118;
    ...
    body.light-theme {
        ...
        --text-main: #3c2f00;
        --accent: #d4a017;
        --accent-glow: #f4c43088;
        --chat-user-bg: linear-gradient(135deg, #fff0c7, #ffe9bf);
        --chat-assist-bg: linear-gradient(135deg, #e6f4ff, #d6eaff);
        --input-bg: #fff8e1;
...
st.caption(f"Reina • {st.session_state.selected_model} • Beehive oracle 🐝 • **Bzzzzt!** Welcome to the hive, human! I'm **Reina** — your slightly sassy beehive oracle.  
